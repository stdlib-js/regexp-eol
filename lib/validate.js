...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0sJ2V,FD', options ) );
	}
	if ( hasOwnProp( options, 'flags' ) ) {
		opts.flags = options.flags;
		if ( !isString( opts.flags ) ) {
			return new TypeError( format( '0sJ2W,Gh', 'flags', opts.flags ) );
		}
	}
	if ( hasOwnProp( options, 'capture' ) ) {
		opts.capture = options.capture;
		if ( !isBoolean( opts.capture ) ) {
			return new TypeError( format( '0sJ2o,GE', 'capture', opts.capture ) );
		}
	}
	return null;
}
